...
默认审批模式是安全模式：命令执行、文件变更和额外权限请求由 `ApprovalCoordinator` 转成 Telegram 一次性审批。为适应手机远程操作场景，Telegram 还提供可选的“本项目内自动允许”模式，但它不是全局放权：
...
- 命令工作目录、文件变更根目录和额外文件系统路径必须位于当前项目目录内；网络权限、项目外路径、未知路径格式和无法确认范围的请求继续走人工审批或拒绝；
