...
from codexrelay.updates import GitHubReleaseUpdateProvider, UpdateChannel
...
def test_stable_provider_skips_drafts_and_prereleases(monkeypatch: Any) -> None:
    payload = [
...
def test_beta_provider_accepts_prerelease(monkeypatch: Any) -> None:
    payload = [{"prerelease": True, "tag_name": "v0.2.0", "html_url": "https://example.test/beta"}]
    monkeypatch.setattr(httpx, "get", lambda *args, **kwargs: FakeResponse(payload))

    provider = GitHubReleaseUpdateProvider(UpdateChannel.BETA)
    state = provider.check_for_updates()

    assert state.available_version == "0.2.0"
    assert state.release_url == "https://example.test/beta"
