...
`/security` 用于查看和修改当前项目的审批模式。安全模式会在需要时通过 Telegram 逐项请求确认；“本项目内自动允许”需要二次确认，并且只允许仍在当前项目目录内、可明确判断范围的操作；网络访问和项目外请求不会被静默允许。切换项目或重新配对后该模式会自动恢复为安全模式。设置保存在 CodexRelay 自己的本地数据库中，不会修改 `~/.codex/config.toml`。
