...
- 记录 Mac 本地生命周期，并在睡眠、唤醒、启动和主动退出时通过 Telegram 通知；
- 离线任务保护：Mac 不在线期间积压的普通任务，恢复后必须明确选择“现在执行”或“忽略”；
...
Mac 进入睡眠或 CodexRelay 正常退出时，已配对的 Telegram 会尽力收到离线通知。应用启动、Mac 唤醒或网络中断超过 30 秒后，CodexRelay 会分别验证 Telegram 与 Codex，确认二者都可用后才通知“已恢复”。`/status` 会显示当前连接生命周期状态、本次在线时长和最近心跳。Mac 离线期间发送的普通任务不会在恢复后静默执行，每条任务都必须选择“现在执行”或“忽略”；状态类命令可以立即响应。断电、强制关机和系统崩溃无法在失联瞬间发送消息，但下次启动会根据最后一次持久化心跳估算离线时间。
