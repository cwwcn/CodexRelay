...
`/help` 会按“常用操作、任务控制、配置与安全、首次设置”分组显示命令。`/security` 用于查看和修改当前项目的审批模式。安全模式会在需要时通过 Telegram 逐项请求确认；“本项目内自动允许”需要二次确认，并且只允许仍在当前项目目录内、可明确判断范围的操作；网络访问和项目外请求不会被静默允许。切换项目或重新配对后该模式会自动恢复为安全模式。设置保存在 CodexRelay 自己的本地数据库中，不会修改 `~/.codex/config.toml`。

项目切换只允许在当前任务结束后进行。需要立刻切换时，先发送 `/stop`，再发送 `/use`。切换项目不会清空原项目对话。

同一项目可以有多个会话。`/sessions` 会同时同步并列出当前项目中“电脑上创建”和“Telegram 创建”的会话，`/session <编号>` 选择会话，`/new` 新建会话。如果项目目录发生过迁移，匹配到的电脑会话会标记为“电脑上创建（路径已迁移）”；选择后会使用当前项目路径恢复原上下文。来源文案表示会话最初在哪一端创建，占用文案则单独表示当前由哪一端持有。选择会话后会由 Telegram 占用它；如果要让电脑端或其他连接器继续使用，请使用 `/release` 释放。会话上下文彼此隔离；模型和推理强度跟随会话保存。`/takeover` 可显式接管空闲会话。应用异常退出后会自动清理没有活动任务的遗留占用。
