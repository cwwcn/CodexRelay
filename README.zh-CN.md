...
选好项目和会话后，直接发送普通文字即可执行任务。需要新开上下文时使用 `/new`；任务运行时需要停止则使用 `/stop`。
...
`/help` 会按“常用操作、任务控制、配置与安全、首次设置”分组显示命令。未配对时仅接受 `/pair <六位配对码>`（也兼容 BotFather 深链格式 `/start pair_<六位配对码>`）；已配对账号再次发送 `/pair` 只会收到状态提示，不会被当作任务执行。`/security` 用于查看和修改当前项目的审批模式，兼容别名为 `/approval`；`/reasoning` 用于修改推理强度，兼容别名为 `/effort`。安全模式会在需要时通过 Telegram 逐项请求确认；“本项目内自动允许”需要二次确认，并且只允许仍在当前项目目录内、可明确判断范围的操作；网络访问和项目外请求不会被静默允许。切换项目或重新配对后该模式会自动恢复为安全模式。设置保存在 CodexRelay 自己的本地数据库中，不会修改 `~/.codex/config.toml`。

项目切换只允许在当前任务结束后进行。需要立刻切换时，先发送 `/stop`，再发送 `/use`。切换项目不会清空原项目对话。

同一项目可以有多个会话。`/sessions` 会同时同步并列出当前项目中“电脑端会话”和“Telegram 创建”的会话，`/session <编号>` 选择会话，`/new` 新建会话。应用启动、后台定期同步和 `/sessions` 请求都会校准会话列表；只要会话在 Codex 中被删除或归档，下一次成功同步时就会从 Telegram 列表中隐藏。如果当前选中的会话失效，CodexRelay 会自动切换到仍有效的第一条会话；没有有效会话时回到未选择状态。本地历史数据会保留用于审计和后续恢复，不会被永久删除。如果会话之后在 Codex 中恢复，Telegram 会按原会话 ID 恢复显示，不会生成重复条目。如果项目目录发生过迁移，匹配到的电脑会话会明确提示路径不可用；选择后会使用当前项目路径恢复原上下文。来源文案表示会话最初在哪一端创建，占用文案只表示当前正在运行的这一轮任务。Telegram 任务完成后会自动释放占用，回到电脑端无需手动交接。会话上下文彼此隔离；模型和推理强度跟随会话保存。`/release` 仅用于清理异常遗留状态，`/takeover` 为兼容保留，不会创建永久锁定。未识别的斜杠命令会直接提示，不会被误当成 Codex 任务。
